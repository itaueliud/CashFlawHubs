...
router.get('/providers/:providerKey/launch', protect, requireActivation, requireTestUserActionAccess('Microtasks'), async (req, res) => {
  try {
...
router.post('/:id/unlock', protect, requireActivation, requireTestUserActionAccess('Microtasks'), deviceFingerprint, async (req, res) => {
  try {
    await ensureBaselineTasks();
...
router.post('/:id/complete', protect, requireActivation, requireTestUserActionAccess('Microtasks'), deviceFingerprint, completionRateLimit, async (req, res) => {
  try {
    await ensureBaselineTasks();
