...
const buildContentSecurityPolicy = (nonce: string) =>
  [
    ...
    `script-src 'self' blob: 'nonce-${nonce}' 'unsafe-inline' 'unsafe-eval' https://challenges.cloudflare.com https://trianglerockers.com https://playabledownload.com ${ADS_ORIGINS.join(' ')}`,
    `script-src-elem 'self' blob: 'nonce-${nonce}' 'unsafe-inline' https://challenges.cloudflare.com https://trianglerockers.com https://playabledownload.com ${ADS_ORIGINS.join(' ')}`,
