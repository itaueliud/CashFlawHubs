...
export default function CreatorHubBrowsePage() {
  ...
  const videoSrc = (item: CreatorUploadItem) => `${item.streamUrl}?token=${encodeURIComponent(token || '')}`;
                  ...
                  {!item.isLocked ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-700 to-slate-900 text-white">
                      <div className="flex flex-col items-center gap-2 text-center">
                        <Lock size={28} />
                        <div className="text-sm font-semibold">{item.priceTokens} Tokens to unlock</div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-300">
                  {item.isLocked ? (
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-700 dark:text-amber-300">{item.priceTokens} Tokens</span>
                    </div>
                  ) : (
