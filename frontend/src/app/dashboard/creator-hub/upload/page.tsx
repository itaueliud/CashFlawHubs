...
const DOCUMENT_ACCEPT = '.pdf,.docx,.pptx,.xlsx,.epub,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/epub+zip';
...
export default function CreatorHubUploadPage() {
  ...
  // Reset tier to a sane default whenever the upload type changes.
  useEffect(() => {
    setTier(uploadType === 'video' ? 'normal' : 'basic');
    setFile(null);
  }, [uploadType]);

  const tiersForType = meta?.tiers?.[uploadType] || {};
  const tierConfig = tiersForType[tier];
  const canAfford = (user?.tokenBalance || 0) >= (tierConfig?.tokenCost || 0);

  const categoryConfig = useMemo(
    () => meta?.categories.find((c) => c.value === category),
    [meta, category],
  );
  ...
  const handleSubmit = async () => {
    if (!file) return toast.error(uploadType === 'video' ? 'Choose a video file' : 'Choose a document file');
    ...
    try {
      ...
      if (data?.auditLogWarning) {
        toast(data.message || 'Upload published', { icon: '!', duration: 5000 });
      ...
      <div className="mx-auto max-w-4xl space-y-5">
        ...
        <div className="flex flex-col gap-3 rounded-[24px] border border-slate-200 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Pick a type, a package, add a title, attach a file, and publish.</p>
          </div>
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-slate-800 dark:text-slate-200">Upload Type</label>
          <div className="grid grid-cols-2 gap-3">
            {meta.uploadTypes.map((type) => (
              <button
                key={type.value}
                type="button"
                onClick={() => setUploadType(type.value)}
                className={`flex items-center gap-3 rounded-2xl border p-4 text-left transition ${uploadType === type.value ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-500/20 dark:bg-emerald-500/10' : 'border-slate-200 bg-white hover:border-emerald-400 dark:border-slate-800 dark:bg-slate-900'}`}
              >
                {type.value === 'video' ? <Video size={22} /> : <FileText size={22} />}
                <div>
                  <div className="font-bold">{type.icon} {type.label}</div>
                  {type.formats && <div className="text-xs text-slate-500">{type.formats.join(', ')}</div>}
                </div>
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {Object.entries(tiersForType).map(([key, value]) => (
              <button
                key={key}
                type="button"
                onClick={() => setTier(key)}
              ...
              >
                ...
                <div className="mt-1 text-xs text-slate-500">
                  up to {value.maxSizeMB}MB{value.maxDurationSec ? ` - ${Math.round(value.maxDurationSec / 60)} mins` : ''}
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <select value={category} onChange={(e) => handleCategoryChange(e.target.value)} className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-400 dark:border-slate-700 dark:bg-slate-900">
              <option value="">Select a category</option>
              {meta.categories.map((item) => (
                <option key={item.value} value={item.value}>{item.label}{item.isNew ? ' (NEW)' : ''}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        {categoryConfig && categoryConfig.subcategories.length > 0 && (
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-800 dark:text-slate-200">Subcategory <span className="text-slate-400 font-normal">(optional)</span></label>
            <select value={subcategory} onChange={(e) => setSubcategory(e.target.value)} className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-400 dark:border-slate-700 dark:bg-slate-900">
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-slate-800 dark:text-slate-200">
            {uploadType === 'video' ? 'Video file' : 'Document file'} {tierConfig ? `(max ${tierConfig.maxSizeMB}MB${tierConfig.maxDurationSec ? `, ${Math.round(tierConfig.maxDurationSec / 60)} mins` : ''})` : ''}
          </label>
          <label className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-3xl border-2 border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500 transition hover:border-emerald-400 dark:border-slate-700 dark:bg-slate-900">
            <UploadCloud size={22} />
            <span>{file ? file.name : uploadType === 'video' ? 'Click to choose a video' : 'Click to choose a PDF, DOCX, PPTX, XLSX, or EPUB'}</span>
            <input
              key={uploadType}
              type="file"
              accept={uploadType === 'video' ? VIDEO_ACCEPT : DOCUMENT_ACCEPT}
              className="hidden"
