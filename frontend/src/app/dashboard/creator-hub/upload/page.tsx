...
export default function CreatorHubUploadPage() {
  ...
  const [defaultPriceTokens, setDefaultPriceTokens] = useState(15);
  ...
  const handleSubmit = async () => {
    ...
    if (isPremium) {
      form.append('defaultPriceTokens', String(defaultPriceTokens));
    }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-800 dark:text-slate-200">Video file {tierConfig ? `(max ${tierConfig.maxSizeMB}MB, ${tierConfig.maxDurationSec}s)` : ''}</label>
      ...
      <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/60">
        ...
        {isPremium && (
          <div className="mt-4 space-y-4">
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Premium price (Tokens)</label>
              <input type="number" min={1} value={defaultPriceTokens} onChange={(e) => setDefaultPriceTokens(Number(e.target.value))} className="w-36 rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none dark:border-slate-700 dark:bg-slate-900" />
            </div>
          </div>
        )}
      </div>
