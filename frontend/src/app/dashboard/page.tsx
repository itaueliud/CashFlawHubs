...
import { ArrowRight, ClipboardList, Zap, Briefcase, Gift, Star, TrendingUp, Flame, Trophy } from 'lucide-react';
import { AreaChart, Area, ResponsiveContainer, Tooltip, XAxis } from 'recharts';

const QUICK_ACTIONS = [
  { href: '/dashboard/surveys', icon: ClipboardList, label: 'Surveys', color: 'bg-blue-500/20 text-blue-400', desc: 'Up to $3 each' },
  ...
  { href: '/dashboard/freelance', icon: TrendingUp, label: 'Freelance', color: 'bg-orange-500/20 text-orange-400', desc: 'Sell your skills' },
  { href: '/dashboard/referrals', icon: Star, label: 'Refer Friends', color: 'bg-green-500/20 text-green-400', desc: '200 KES each' },
];
...
export default function DashboardPage() {
  ...
  // Fake sparkline data
  const chartData = [
    { day: 'Mon', earn: 0.4 }, { day: 'Tue', earn: 1.2 }, { day: 'Wed', earn: 0.8 },
    { day: 'Thu', earn: 2.1 }, { day: 'Fri', earn: 1.5 }, { day: 'Sat', earn: 3.2 }, { day: 'Sun', earn: 1.8 },
  ];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {!user?.activationStatus && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 flex items-center justify-between">
          <div>
            ...
            <div className="text-sm text-slate-400 mt-0.5">Pay 500 KES to unlock surveys, tasks, and referral earnings</div>
          </div>
      ...
      {/* Quick actions */}
      <div>
        <h2 className="font-bold text-lg mb-3">Earn Money</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {QUICK_ACTIONS.map((a) => (
            <Link key={a.href} href={a.href}
              className="card hover:border-slate-500 transition-all group flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl ${a.color} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}>
                <a.icon size={18} />
              </div>
              <div>
            ...
            {challenges.slice(0, 4).map((c: any) => (
              <div key={c._id} className="card">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="badge-green">${c.rewardUSD}</div>
                </div>
                <p className="text-xs text-slate-400 mb-3">{c.description}</p>
                <div className="flex items-center gap-2">
                  <div className="flex-1 bg-slate-700 rounded-full h-1.5">
                    <div className="bg-green-500 h-1.5 rounded-full transition-all"
                      style={{ width: `${Math.min((c.progress / c.targetCount) * 100, 100)}%` }} />
                  </div>
                  <span className="text-xs text-slate-400">{c.progress}/{c.targetCount}</span>
                  {c.completed && !c.rewardClaimed && (
                    <button className="btn-primary text-xs py-1 px-2">Claim!</button>
                  )}
                  {c.rewardClaimed && <span className="text-xs text-green-400">✓ Claimed</span>}
                </div>
              </div>
            ))}
          </div>
