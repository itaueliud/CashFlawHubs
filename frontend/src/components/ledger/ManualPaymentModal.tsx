...
export default function ManualPaymentModal({ onClose, onSuccess }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-blue-500/20 bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="flex items-center gap-3 text-green-300">
              ...
              <div>
                ...
                <div className="text-xs text-green-400">
                  New balance: ${Number(mutation.data?.transaction?.newBalance || 0).toFixed(2)}
                </div>
              </div>
            </div>
            <button
              onClick={onClose}
              className="mt-4 w-full rounded-xl bg-green-500/20 py-2 text-sm font-semibold text-green-300 hover:bg-green-500/30"
            >
              Close
            </button>
          </div>
        ...
        {!mutation.isSuccess && (
          <div className="mt-5 space-y-4">
            <div>
              ...
              <input
                type="text"
                placeholder="user@email.com or CFH-XXXX"
                value={form.userIdentifier}
                onChange={(event) => setForm((current) => ({ ...current, userIdentifier: event.target.value }))}
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) => setForm((current) => ({ ...current, amountLocal: event.target.value }))}
                  className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. QKA1B2C3D4 (M-Pesa code)"
                value={form.providerReference}
                onChange={(event) => setForm((current) => ({ ...current, providerReference: event.target.value.trim() }))}
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none"
              />
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full resize-none rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none"
              />
            </div>
            ...
            <button
              ...
              className="w-full rounded-xl bg-blue-600 py-3 text-sm font-bold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-40"
            >
              {mutation.isPending ? (
